...
def get_snp_dfs(min_threshold, fileName):
    names = get_vcf_names(fileName)
    names
    # vcf = pd.read_csv(fileName, comment='#', chunksize=1000000, delim_whitespace=True, header=None, names=names)
    vcf = pd.read_csv(fileName, comment='#', nrows=10000, skiprows=0,
